...
// Java에서는 private static final로 클래스 레벨 로거를 선언한다
@Service
public class EventPublisher {
    ...
    // Kotlin에서는 타입 파라미터를 <ChannelEvent>로 메서드 체인 끝에 명시하지만,
    // Java에서는 변수 타입 Sinks.Many<ChannelEvent>로 제네릭이 추론된다
    private final Sinks.Many<ChannelEvent> sinks = Sinks.many()
        .multicast()
    ...
    // isFailure(): EmitResult의 메서드 — 발행 실패 여부를 확인한다
    //   Kotlin에서는 result.isFailure (프로퍼티 접근 스타일)이지만,
    //   Java에서는 result.isFailure() (메서드 호출 스타일)이다
    public void publish(ChannelEvent event) {
        Sinks.EmitResult result = sinks.tryEmitNext(event); // 이벤트 발행 시도
