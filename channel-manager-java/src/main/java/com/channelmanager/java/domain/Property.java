...
public class Property {
    ...
    @Schema(description = "숙소 고유 식별자 (PK)", example = "1") // PK 문서화
    @Id // 이 필드가 PK(Primary Key)임을 Spring Data에 알린다
    private Long id; // PK - null이면 새 엔티티(INSERT), 값이 있으면 기존 엔티티(UPDATE)

    @Schema(description = "숙소명", example = "서울 그랜드 호텔") // 숙소 이름 문서화
    private String name; // 숙소명 (예: "서울 그랜드 호텔")

    @Schema(description = "숙소 주소", example = "서울특별시 중구 을지로 30", nullable = true) // 선택 입력 필드 문서화
    private String address; // 숙소 주소 - nullable (선택 입력)
