...
public class Inventory {
    ...
    @Schema(description = "재고 고유 식별자 (PK)", example = "1") // PK 문서화
    @Id // 이 필드가 PK임을 Spring Data에 알린다
    private Long id; // PK - null이면 INSERT, 값이 있으면 UPDATE

    @Schema(description = "객실 타입 ID (FK)", example = "1") // 외래키 문서화
    private Long roomTypeId; // 객실 타입 FK - 어떤 객실 타입의 재고인지 식별

    @Schema(description = "재고 날짜", example = "2026-03-15") // 날짜 문서화
    private LocalDate stockDate; // 재고 날짜 - 이 날짜의 재고 현황을 나타낸다

    @Schema(description = "전체 객실 수", example = "10") // 전체 수량 문서화
    private int totalQuantity; // 전체 객실 수 - 해당 날짜에 판매 가능한 총 객실 수

    @Schema(description = "예약 가능 수량", example = "5") // 잔여 수량 문서화
