...
import com.channelmanager.java.service.CacheService; // Phase 18: Redis 캐시 서비스
...
public class StatisticsController {
    ...
    private final CacheService cacheService;           // Phase 18: Redis 캐시 서비스
    ...
    public Flux<ChannelStatistics> getChannelStatistics() {
        return cacheService.getOrLoad("channels", ChannelStatistics[].class,
                () -> statisticsService.getChannelStatistics().collectList()
                    .map(list -> list.toArray(new ChannelStatistics[0])))
            .flatMapIterable(java.util.Arrays::asList); // Array → Flux 변환
    }
    ...
    public Flux<EventStatistics> getEventStatistics() {
        return cacheService.getOrLoad("events", EventStatistics[].class,
                () -> statisticsService.getEventStatistics().collectList()
                    .map(list -> list.toArray(new EventStatistics[0])))
            .flatMapIterable(java.util.Arrays::asList);
    }
    ...
    public Flux<RoomTypeStatistics> getRoomTypeStatistics() {
        return cacheService.getOrLoad("rooms", RoomTypeStatistics[].class,
                () -> statisticsService.getRoomTypeStatistics().collectList()
                    .map(list -> list.toArray(new RoomTypeStatistics[0])))
            .flatMapIterable(java.util.Arrays::asList);
    }
    ...
    public Mono<SummaryStatistics> getSummaryStatistics() {
        return cacheService.getOrLoad("summary", SummaryStatistics.class,
