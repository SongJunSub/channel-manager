...
//   - 직렬화 크기가 작다 (성능)
// Kotlin에서는 class RedisConfig이지만, Java에서는 public class RedisConfig이다
@Configuration
public class RedisConfig {
    ...
    // Kotlin에서는 fun reactiveRedisTemplate(...)이지만,
    // Java에서는 public ReactiveRedisTemplate<String, String> reactiveRedisTemplate(...)이다
    @Bean
    @Primary // Spring Boot 자동설정의 ReactiveRedisTemplate보다 우선 사용
    public ReactiveRedisTemplate<String, String> reactiveStringRedisTemplate(
            ReactiveRedisConnectionFactory connectionFactory) { // 자동 주입되는 커넥션 팩토리
    ...
    // Kotlin에서는 jacksonObjectMapper()로 Kotlin 모듈을 포함하지만,
    // Java에서는 new ObjectMapper()로 생성하고 JavaTimeModule만 등록한다
    @Bean
    public ObjectMapper objectMapper() {
        return new ObjectMapper()
            .registerModule(new JavaTimeModule()) // Java 8 날짜/시간 직렬화 모듈
