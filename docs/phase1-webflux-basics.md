...
```kotlin
// Kotlin 예시
fun findPropertyById(id: Long): Mono<Property> {
    return propertyRepository.findById(id)  // 결과가 0개(없음) 또는 1개
}
```

```java
// Java 예시
public Mono<Property> findPropertyById(Long id) {
    return propertyRepository.findById(id); // 결과가 0개(없음) 또는 1개
}
```
...
```kotlin
// Kotlin 예시
fun findAllProperties(): Flux<Property> {
    return propertyRepository.findAll()  // 0개 ~ N개의 숙소 목록
}
```

```java
// Java 예시
public Flux<Property> findAllProperties() {
    return propertyRepository.findAll(); // 0개 ~ N개의 숙소 목록
}
```
...
Mono.just(1L)
    .flatMap { id -> propertyRepository.findById(id) }  // Mono<Property>
```
...
Mono.zip(
    propertyRepository.findById(1L),          // Mono<Property>
    roomTypeRepository.findByPropertyId(1L)   // Flux<RoomType> → collectList → Mono<List>
).map { tuple ->
    PropertyDetail(tuple.t1, tuple.t2)        // 두 결과를 합쳐서 새 객체 생성
}
```
...
propertyRepository.findById(id)
    .onErrorReturn(Property.empty())

// onErrorResume: 에러 시 대체 Publisher로 전환
propertyRepository.findById(id)
    .onErrorResume { error ->
        Mono.error(NotFoundException("Property not found: $id"))
    }
```
...
```kotlin
// ❌ 이 코드는 아무 일도 하지 않는다!
propertyRepository.findById(1L)    // Mono를 만들었지만 구독하지 않음

// ✅ 구독해야 실제로 실행된다
propertyRepository.findById(1L)
    .subscribe { property -> println(property) }
```

**Spring WebFlux에서는 프레임워크가 자동으로 구독한다:**
```kotlin
@GetMapping("/properties/{id}")
fun getProperty(@PathVariable id: Long): Mono<Property> {
    return propertyRepository.findById(id)   // 반환만 하면 WebFlux가 자동 구독
}
```
...
val property = propertyRepository.findById(1L)
val roomTypes = property.roomTypes  // 자동으로 추가 쿼리 실행 (블로킹)

// R2DBC 방식 (명시적으로 조회하고 조합)
propertyRepository.findById(1L)
    .flatMap { property ->
        roomTypeRepository.findByPropertyId(property.id!!)
            .collectList()
            .map { roomTypes -> PropertyDetail(property, roomTypes) }
    }
```
...
interface PropertyRepository : ReactiveCrudRepository<Property, Long> {
    // 기본 제공 메서드:
    // findById(id): Mono<Property>
    // findAll(): Flux<Property>
    // save(entity): Mono<Property>
    // deleteById(id): Mono<Void>

    // 커스텀 쿼리 메서드
    fun findByName(name: String): Mono<Property>
}
```
...
├── V1__create_properties_table.sql        # 버전 1: 숙소 테이블 생성
...
```
[Property] 1 ──── N [RoomType] 1 ──── N [Inventory]
                                          |
                                          N
                                     [Reservation]
                                          |
...
CREATE TABLE properties (
    id          BIGSERIAL PRIMARY KEY,       -- 숙소 PK (자동 증가)
...
CREATE TABLE room_types (
    ...
    created_at  TIMESTAMP DEFAULT NOW(),     -- 생성 시각
    CONSTRAINT fk_room_types_properties FOREIGN KEY (property_id) REFERENCES properties(id)
);
...
- `toString()` — `Property(id=1, name=서울 그랜드 호텔, ...)`
- `copy()` — 일부 필드만 변경한 복사본 생성: `property.copy(name = "새 이름")`
- `componentN()` — 구조 분해 선언: `val (id, name) = property`

#### Java: Lombok 어노테이션

```java
@Data                  // getter, setter, toString, equals, hashCode 자동 생성
@Builder               // 빌더 패턴: Property.builder().name("...").build()
...
- `@Builder` → `Property.builder().name("서울 그랜드 호텔").address("서울시").build()`
- `@NoArgsConstructor` → `new Property()` (인자 없는 기본 생성자)
- `@AllArgsConstructor` → `new Property(id, name, address, createdAt)` (모든 필드 생성자)
...
| **객체 복사** | `property.copy(name = "새 이름")` 내장 | 직접 구현하거나 `@Builder`의 `toBuilder` 사용 |
...
| **구조 분해** | `val (id, name) = property` 기본 지원 | 미지원 |
...
// 일부 필드만 변경한 복사본 생성
val updated = property.copy(name = "부산 그랜드 호텔")
```
...
// JPA 방식 (사용 불가)
@ManyToOne
private Property property;           // 객체 참조

// R2DBC 방식 (이 프로젝트)
private Long propertyId;          // ID 값만 저장, 필요 시 별도 쿼리로 조회
```
