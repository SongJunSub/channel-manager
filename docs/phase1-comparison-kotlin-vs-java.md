...
var property = new Property();          // 타입을 추론하지만 가변이다
...
// === Kotlin: 대부분 타입 추론 가능 ===
val id = 1L                       // Long으로 추론
val properties = listOf("A", "B")    // List<String>으로 추론
val result = propertyRepository.findById(1L)  // Mono<Property>로 추론

// 함수 반환 타입은 public API에서는 명시 권장
fun findProperty(id: Long): Mono<Property> = propertyRepository.findById(id)
```

```java
// === Java: var 외에는 타입 명시 필수 ===
long id = 1L;                     // 타입 명시
List<String> properties = List.of("A", "B");  // 타입 명시
Mono<Property> result = propertyRepository.findById(1L);  // 타입 명시

// Java 10+ var (로컬 변수에서만 사용 가능)
var property = propertyRepository.findById(1L);  // Mono<Property>로 추론
...
fun getPropertyAddress(property: Property): String {
    return property.address ?: "주소 없음"  // Elvis 연산자로 기본값 제공
}
```
...
public String getPropertyAddress(Property property) {
    return property.getAddress() != null ? property.getAddress() : "주소 없음";
}
```
...
| 연산자 | 이름 | 설명 | 예시 |
|---|---|---|---|
| `?.` | Safe call | null이면 null 반환, 아니면 메서드 실행 | `property?.name` |
...
```kotlin
// === Kotlin: 간결한 클래스 정의 ===
class PropertyService(                           // primary constructor에서 프로퍼티 선언
    private val propertyRepository: PropertyRepository  // val: 불변 프로퍼티 + 생성자 파라미터
) {
    fun findById(id: Long): Mono<Property> {     // fun으로 함수 정의
...
public class PropertyService {
    ...
    private final PropertyRepository propertyRepository;  // final: 불변 필드

    public PropertyService(PropertyRepository propertyRepository) {  // 생성자 직접 정의
        this.propertyRepository = propertyRepository;              // this로 필드에 할당
    }

    public Mono<Property> findById(Long id) {     // public 메서드
        return propertyRepository.findById(id);
    }
}
// - 접근 제한자 명시 필수
...
val properties = listOf(
    Property(id = 1, name = "서울 호텔", address = "서울"),
    Property(id = 2, name = "부산 호텔", address = "부산"),
    Property(id = 3, name = "제주 호텔", address = null)
)
...
// groupBy
val byAddress = properties
    .filterNot { it.address == null }
    .groupBy { it.address }
// {"서울": [Property(...)], "부산": [Property(...)]}
```

```java
// === Java: Stream API 사용 ===
List<Property> properties = List.of(
    Property.builder().id(1L).name("서울 호텔").address("서울").build(),
    Property.builder().id(2L).name("부산 호텔").address("부산").build(),
    Property.builder().id(3L).name("제주 호텔").address(null).build()
);

// 필터 + 변환 + 정렬
List<String> result = properties.stream()
    .filter(h -> h.getAddress() != null)        // null이 아닌 것만
    .map(Property::getName)                        // 이름만 추출
    .sorted()                                   // 정렬
    .toList();                                  // Java 16+
// result: ["부산 호텔", "서울 호텔"]

// groupBy
Map<String, List<Property>> byAddress = properties.stream()
    .filter(h -> h.getAddress() != null)
    .collect(Collectors.groupingBy(Property::getAddress));
```
...
val property: Property? = findProperty(id)
property?.let {
    println("숙소명: ${it.name}")     // it = property (non-null)
}

// 변환에 사용
val propertyName: String? = findProperty(id)?.let { it.name }
```
...
val property = Property(name = "서울 호텔").apply {
    // this = Property 객체 (this 생략 가능)
    println("숙소 생성: $name")
}
```
...
val property = propertyRepository.save(property)
    .also { println("저장된 숙소: ${it.name}") }  // 로깅 후 property 반환
```
...
// let → Optional.map()
Optional.ofNullable(property).map(h -> h.getName());

// also → 별도 변수 할당 + 로깅
Property saved = propertyRepository.save(property);
System.out.println("저장된 숙소: " + saved.getName());
```
...
class PropertyService(
    private val propertyRepository: PropertyRepository,    // 생성자 주입 (자동)
    private val roomTypeRepository: RoomTypeRepository
) {
    fun findAll(): Flux<Property> = propertyRepository.findAll()
}
...
public class PropertyService {
    private final PropertyRepository propertyRepository;        // final → 생성자 주입 대상
...
public class PropertyService {
    ...
    public PropertyService(PropertyRepository propertyRepository) {  // 생성자 직접 작성
...
class PropertyController(
    ...
    @GetMapping
    fun findAll(): Flux<Property> = propertyService.findAll()  // 단일 표현식 함수

    @GetMapping("/{id}")
    fun findById(@PathVariable id: Long): Mono<Property> =
        propertyService.findById(id)

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    fun create(@RequestBody request: CreatePropertyRequest): Mono<Property> =
...
public class PropertyController {
    ...
    @GetMapping("/{id}")
    public Mono<Property> findById(@PathVariable Long id) {
