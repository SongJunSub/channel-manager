...
// @Order(Int.MIN_VALUE + 1): RequestLoggingFilter(MIN_VALUE) 다음에 실행
...
class RateLimitFilter : WebFilter {
    ...
    private var capacity: Long = 50         // 버킷 최대 용량 (초당 최대 50건 버스트)

    @org.springframework.beans.factory.annotation.Value("\${rate-limit.refill-tokens:50}")
    private var refillTokens: Long = 50     // 충전할 토큰 수

    private val refillDuration: Duration = Duration.ofSeconds(1) // 충전 주기 (1초)
    ...
    override fun filter(exchange: ServerWebExchange, chain: WebFilterChain): Mono<Void> {
        ...
        return if (bucket.tryConsume(1)) {
            ...
            createRateLimitResponse(exchange)
        }
    }
    ...
    private fun createRateLimitResponse(exchange: ServerWebExchange): Mono<Void> {
        val response = exchange.response
        response.statusCode = HttpStatus.TOO_MANY_REQUESTS            // 429 상태 코드
        response.headers.set("Retry-After", "1")                      // 1초 후 재시도 안내
        response.headers.contentType = MediaType.APPLICATION_JSON     // JSON 응답

        // JSON 응답 본문 — ErrorResponse 형식과 동일
        val body = """{"message":"요청 횟수 제한을 초과했습니다. 잠시 후 다시 시도해주세요."}"""
        val buffer = response.bufferFactory().wrap(body.toByteArray(Charsets.UTF_8)) // 문자열을 UTF-8 DataBuffer로 변환
