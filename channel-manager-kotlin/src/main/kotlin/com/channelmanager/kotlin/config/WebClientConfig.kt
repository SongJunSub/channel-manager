...
class WebClientConfig {
    ...
    // baseUrl: 모든 요청의 기본 URL을 설정한다 (예: http://localhost:8080)
    // 시뮬레이터는 자기 자신의 API를 호출하므로 localhost + 현재 서버 포트를 사용한다
    @Bean
    fun webClient(
        @Value("\${server.port:8080}") port: Int // application.yml의 server.port 값, 기본값 8080
    ): WebClient =
        WebClient.builder() // WebClient 빌더 시작
