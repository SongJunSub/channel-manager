...
data class Reservation(
    @field:Schema(description = "예약 고유 식별자 (PK)", example = "1") // PK 문서화
    @Id // 이 필드가 PK임을 Spring Data에 알린다
    val id: Long? = null, // PK - null이면 INSERT, 값이 있으면 UPDATE

    @field:Schema(description = "예약 채널 ID (FK)", example = "1") // 채널 외래키 문서화
    val channelId: Long, // 채널 FK - 어떤 채널을 통해 예약되었는지 식별

    @field:Schema(description = "객실 타입 ID (FK)", example = "1") // 객실 타입 외래키 문서화
    val roomTypeId: Long, // 객실 타입 FK - 어떤 객실 타입을 예약했는지 식별

    @field:Schema(description = "체크인 날짜", example = "2026-03-15") // 체크인 문서화
    val checkInDate: LocalDate, // 체크인 날짜

    @field:Schema(description = "체크아웃 날짜", example = "2026-03-17") // 체크아웃 문서화
    val checkOutDate: LocalDate, // 체크아웃 날짜

    @field:Schema(description = "투숙객 이름", example = "홍길동") // 투숙객 문서화
    val guestName: String, // 투숙객 이름

    @field:Schema(description = "예약 객실 수", example = "1", defaultValue = "1") // 객실 수 문서화
    val quantity: Int = 1, // 예약 객실 수 - 기본값 1개

    @field:Schema(description = "예약 상태 (CONFIRMED, CANCELLED)", example = "CONFIRMED") // 상태 문서화
    val status: String, // 예약 상태 - ReservationStatus enum의 name() 값을 저장한다

    @field:Schema(description = "총 금액", example = "300000", nullable = true) // 금액 문서화
