...
class EventStreamControllerTest {
    ...
    private lateinit var eventStreamController: EventStreamController
    ...
    fun `SSE 스트림 - 이벤트 발행 시 SSE로 수신한다`() {
        // 테스트 시나리오:
        ...
        val savedEvent = channelEventRepository.save(
            ChannelEvent(
        ...
        StepVerifier.create(
            ...
            .assertNext { sse -> // 수신된 SSE 이벤트를 검증한다
                ...
                assertThat(sse.id()).isEqualTo(savedEvent.id.toString())
    ...
    fun `SSE 스트림 - 여러 이벤트 타입을 순서대로 수신한다`() {
        ...
        createdEventIds.add(event1.id!!)

        val event2 = channelEventRepository.save(
            ChannelEvent(
        ...
        StepVerifier.create(
            ...
            .assertNext { sse -> // 두 번째 이벤트: CHANNEL_SYNCED
                ...
                assertThat(sse.id()).isEqualTo(event2.id.toString())
    ...
    @Test // SSE 이벤트에 heartbeat가 포함되어 있는지 확인
    @Order(3) // 세 번째로 실행
    fun `SSE 스트림 - heartbeat 이벤트가 포함되어 있다`() {
    ...
    @Order(4) // 네 번째로 실행
    fun `최근 이벤트 조회 - 기본 limit으로 이벤트 목록을 반환한다`() {
        ...
        webTestClient.get()
            ...
            .expectBodyList(EventResponse::class.java)
            .consumeWith<WebTestClient.ListBodySpec<EventResponse>> { result ->
                ...
                for (i in 0 until events.size - 1) {
                    val current = events[i].createdAt  // 현재 이벤트의 생성 시각
                    val next = events[i + 1].createdAt // 다음 이벤트의 생성 시각
    ...
    @Order(5) // 다섯 번째로 실행
    fun `최근 이벤트 조회 - limit 파라미터로 개수를 제한한다`() {
    ...
    @Order(6) // 여섯 번째로 실행
    fun `최근 이벤트 조회 - 응답에 필수 필드가 포함되어 있다`() {
