    ...
    fun `이벤트를 발행하면 구독자가 수신한다`() {
        ...
        StepVerifier.create(eventPublisher.getEventStream().take(1)) // 1개만 수신
            ...
            .expectNextMatches { received -> // 수신 이벤트 검증
                received.eventType == EventType.RESERVATION_CREATED // 이벤트 타입 확인
                    && received.channelId == 1L // 채널 ID 확인
            }
            .verifyComplete() // 스트림 완료 확인
    }

    @Test // 여러 구독자가 동시에 같은 이벤트를 수신하는지 확인 (multicast 검증)
    fun `여러_구독자가_동시에_이벤트를_수신한다`() {
        val event = ChannelEvent(
        ...
        val received1 = mutableListOf<ChannelEvent>() // 구독자 1 수신 버퍼
        val received2 = mutableListOf<ChannelEvent>() // 구독자 2 수신 버퍼

        // 구독자 1: 이벤트 타입을 수집
        val disposable1 = eventPublisher.getEventStream()
            .take(1)
            .subscribe { received1.add(it) }

        // 구독자 2: 같은 Sinks에서 수신
        val disposable2 = eventPublisher.getEventStream()
            .take(1)
            .subscribe { received2.add(it) }
        ...
        assertThat(received1[0].eventType).isEqualTo(EventType.INVENTORY_UPDATED)
        assertThat(received2).hasSize(1) // 구독자 2: 1개 수신
        assertThat(received2[0].roomTypeId).isEqualTo(2L)
    ...
    fun `이벤트_스트림에서_특정_타입만_필터링하여_수신한다`() {
        ...
        StepVerifier.create(
            eventPublisher.getEventStream()
                .filter { it.eventType == EventType.INVENTORY_UPDATED } // 재고 변경만 필터
                .take(1) // 1개만 수신
        )
            .then {
            ...
            .expectNextMatches { it.eventType == EventType.INVENTORY_UPDATED } // 필터 동작 확인
            .verifyComplete()
    }
}
